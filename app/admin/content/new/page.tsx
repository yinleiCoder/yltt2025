...
export default async function NewContentPage({ searchParams }: { searchParams: Promise<{ kind?: string }> }) {
  ...
  return <main className="container mx-auto w-full px-4 py-6 sm:px-6 lg:px-8 lg:py-8"><header className="border-b pb-6"><p className="text-sm text-muted-foreground">内容管理</p><h1 className="mt-1 text-2xl font-semibold">{title}</h1></header><NewContentForm kind={kind} /></main>;
}
