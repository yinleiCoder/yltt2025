...
export function ContentForm({ action, initialValues, mode }: ContentFormProps) {
  ...
  return (
    <form aria-busy={isPending || isUploading || isSubmitting} className="grid w-full min-w-0 max-w-3xl gap-6 py-8" onSubmit={(event) => { event.preventDefault(); uploadAndSubmit(new FormData(event.currentTarget)); }}>
      {initialValues?.id ? <input name="id" type="hidden" value={initialValues.id} /> : null}
      {isEdit ? <input name="kind" type="hidden" value={kind} /> : null}
      {kind === "story" ? <section aria-labelledby="story-settings-title" className="grid min-w-0 gap-4 rounded-lg border bg-muted/10 p-4"><div className="grid gap-1"><h2 className="text-sm font-medium" id="story-settings-title">故事设置</h2><FieldDescription>选择故事发生日期，公开故事会按这个日期从近到远排列。</FieldDescription></div><Field><FieldLabel htmlFor="occurredAt">故事发生日期</FieldLabel><Input id="occurredAt" name="occurredAt" onChange={(event) => setOccurredAt(event.target.value)} type="date" value={occurredAt} required /></Field></section> : null}
      <FieldGroup className="grid min-w-0 gap-5 md:grid-cols-2 [&>*]:min-w-0">
        <Field><FieldLabel htmlFor="kind">内容类型</FieldLabel><Select disabled={isEdit} name={isEdit ? undefined : "kind"} value={kind} onValueChange={(value) => handleKindChange(value as ContentFormInitialValues["kind"])}><SelectTrigger className="w-full" id="kind"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="photo">摄影</SelectItem><SelectItem value="video">短片</SelectItem><SelectItem value="story">故事</SelectItem></SelectContent></Select></Field>
      ...
      {kind !== "story" ? <Field><FieldLabel>{isEdit ? "替换媒体（可选）" : kind === "photo" ? "照片文件" : "短片文件"}</FieldLabel><FieldDescription>{kind === "photo" ? "支持 JPEG、PNG、WebP，单个文件不超过 200 MB。" : "支持 MP4，单个文件不超过 2 GB，文件将直接上传。"}</FieldDescription><MediaFilePreview kind={kind} onFile={(file) => void handleMediaChange(file)} onClear={() => { setMediaFile(null); setVideoMetadata(null); setVideoMetadataStatus(null); }} onError={setUploadError} />{uploadProgress !== null ? <Progress aria-label="媒体上传进度" value={uploadProgress}><ProgressLabel>正在上传媒体</ProgressLabel><ProgressValue /></Progress> : null}</Field> : null}
      {kind === "photo" ? <div className="grid min-w-0 gap-4 sm:grid-cols-2 [&>*]:min-w-0">
      ...
      <div className="grid gap-4 border-t pt-5">
        <Field><FieldLabel htmlFor="locationVisibility">地点公开范围</FieldLabel><Select name="locationVisibility" value={locationVisibility} onValueChange={(value) => setLocationVisibility(value as NonNullable<ContentFormInitialValues["locationVisibility"]>)}><SelectTrigger className="w-full" id="locationVisibility"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="hidden">不公开</SelectItem><SelectItem value="city">仅城市和地区</SelectItem><SelectItem value="precise">公开精确地点</SelectItem></SelectContent></Select></Field>
        ...
        {locationVisibility !== "hidden" ? <div className="grid min-w-0 gap-4 sm:grid-cols-2 [&>*]:min-w-0">
